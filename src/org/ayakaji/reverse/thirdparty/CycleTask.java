...
public class CycleTask extends Task {
	public static final int cycleTaskDelaySec = 1000;
	public static final int cycleTaskPeriodSec = 5000;
	private static final String[] memFilter = new String[] { "mem", "used", "free", "total", "usedPercent" };

	@Override
	public void run() {
		UdpClient.send(merge(new String[] { getMem(), getSwap(), getCpuPerc(), getWho(), getFsUsage() }));
	}
	...
	private static String getMem() {
		...
		return JSON.toJSONString(jsonObj, getIncludeFilter(memFilter), SerializerFeature.PrettyFormat);
	}
